// Request and response types for the JSON API, mirroring
// backend/app/schemas.py. Pointer fields serialize as null when absent, the
// same way pydantic serializes optional fields.
package api
...
// AuthResponse carries the minted Coder session token.
type AuthResponse struct {
	SessionToken string `json:"session_token"`
}

// MeResponse describes the current Coder user.
...
// WorkspaceAccessResponse describes how a workspace can be accessed.
...
// VSCodeDesktopResponse carries the vscode:// deep link.
type VSCodeDesktopResponse struct {
	URI string `json:"uri"`
}

// RichParameterValue is one template parameter for workspace creation.
...
// Validate replicates the pydantic constraints on CreateWorkspaceRequest.
...
// ToCoderBody builds the Coder API request body for workspace creation.
...
// BuildSummary summarizes a workspace's latest build.
...
// WorkspaceResponse is the API's workspace representation.
...
// WorkspaceListResponse wraps the workspace list.
type WorkspaceListResponse struct {
	Count      int64               `json:"count"`
	Workspaces []WorkspaceResponse `json:"workspaces"`
}

// WorkspaceBuildResponse is the API's workspace build representation.
...
// ProvisionerJobLog is one Terraform / provisioner log line.
...
// WorkspaceAgentLog is one agent / startup-script log line.
...
func WorkspaceBuildResponseFromCoder(payload map[string]any) WorkspaceBuildResponse {
	...
	return WorkspaceBuildResponse{
		...
		JobError:    optString(job, "error"),
		CreatedAt:   optString(payload, "created_at"),
	}
}

// ProvisionerJobLogFromCoder maps one provisioner log entry.
...
// WorkspaceAgentLogFromCoder maps one agent log entry.
...
// WorkspaceAccess mirrors main.py _workspace_access: it derives every access
// flag and link for one workspace from the raw Coder payloads.
