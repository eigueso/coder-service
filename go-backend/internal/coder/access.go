// Helpers for resolving workspace agents and building access links,
...
// IterAgents collects every agent across the latest build's resources.
...
// FindApp returns the agent app with the given slug, or nil.
...
// DisplayApps returns the agent's display_apps entries as strings.
...
// IsStarted reports whether the latest build succeeded with a start
// transition.
...
// AgentLifecycleState returns the agent's lifecycle_state, or empty.
func AgentLifecycleState(agent map[string]any) string {
	return getString(agent, "lifecycle_state")
}

// IsAgentReady is true once the agent startup script has finished and the
// agent is ready.
func IsAgentReady(agent map[string]any) bool {
	return AgentLifecycleState(agent) == "ready"
}

// BuildVSCodeBrowserURL builds the path-based code-server app URL:
// {base}/@{username}/{workspace}.{agent}/apps/{slug}/
func BuildVSCodeBrowserURL(baseURL, username, workspaceName, agentName, appSlug string) string {
	...
	return base + "/@" + url.PathEscape(username) + "/" +
		...
		"/apps/" + url.PathEscape(appSlug) + "/"
}

// BuildWebTerminalURL builds the Coder dashboard web terminal URL:
// {base}/@{username}/{workspace}[.{agent}]/terminal
...
// BuildVSCodeDesktopURI builds the vscode://coder.coder-remote/open?... deep
// link used by the VS Code Desktop extension.
